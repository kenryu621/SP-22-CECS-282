...
void myDate::display() {
	std::cout << this->myDate::getDate() << std::endl;
}
...
std::string myDate::getDate() {
	...
	return month_name + " " + std::to_string(this->day) + ", " + std::to_string(this->year);
}

bool myDate::operator>(myDate right) {
	return Greg_to_Jul(this->year, this->month, this->day) > Greg_to_Jul(right.getYear(), right.getMonth(), right.getDay());
}
